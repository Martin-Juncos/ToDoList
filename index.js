...
// innerText recupera y establece el contenido de la etiqueta como texto plano, 
// mientras que innerHTML recupera y establece el mismo contenido pero en formato HTML
...
// agrego al innerHTML del span con mi nombre
...
function ToDo (description) { // clase constructora
  // Tu código acá:
  this.description = description; // se pinta el argumento cuando lo pongo como valor
  this.complete = false;
}
...
function buildToDo(todo, index) { // ya estan agregados los parametros
  // Tu código acá:
  // {descripcion: 'Hola', complete:false}
  // paso 1 y 2
  var toDoShell = document.createElement('div'); // creamos un div
  toDoShell.setAttribute('class', 'toDoShell');
  // paso 3
  var toDoText = document.createElement('span'); // creamos un span
  // paso 4
  toDoText.innerHTML = todo.description; //al innerhtml del span que acabo de crear seteale el valor de descripcion
  // paso 5
  toDoText.setAttribute('id', index); // seteamos un id al span
  // paso 6
  if(todo.complete){ // si esta en true 
...
// Tu código acá:
//              seleccionamos             agregamos el escuchador
document.querySelector('#addButton').addEventListener('click', addToDo) 
// le paso la func solo definida, cuando se haga click se ejecuta
// en vez de pasarle la funcion le estariamos pasando lo que nos devuelve esa func()
// podemos fijarnos en la aplicacion en el navegador
...
function completeToDo(event) {
  // DESCOMENTAR LA SIGUIENTE LINEA
   const index = event.target.id; // tengo aca el id del elemento que disparo el evento
  // Tu código acá:
  // paso 1 toItems que era? un array de objetos Todo
  toDoItems[index].completeToDo() // paso el complete a true!
  //paso 2
  displayToDos() // va llamando a las otras funciones, guardando los elementos y todo eso
  //paso 3
